...
async def calculate_booking_cost(request: BookingCostRequest) -> BookingCostResponse:
    ...
    price_per_seat_inr = Decimal(str(
        event_run.get("special_pricing_inr") or 
        event_run["experiences"]["price_inr"]
    ))
    ...
    try:
        payment_wei, stake_wei, total_wei = await blockchain_service.calculate_booking_cost(
...
async def complete_event_blockchain(request: CompleteEventRequest) -> CompleteEventResponse:
    """
    ...
    event_response = (
        service_client.table("event_runs")
        .select("*, experiences(host_id, users!experiences_host_id_fkey(wallet_address))")
        .eq("id", request.event_run_id)
        .execute()
    )
    ...
    if not blockchain_event_run_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Event not on blockchain"
        )
    ...
    no_show_ids = [b["id"] for b in all_bookings if b["id"] not in request.attended_booking_ids]
    if no_show_ids:
        service_client.table("event_run_bookings").update({
...
async def convert_inr_to_wei(
    ...
    """Convert INR amount to Wei."""
    wei_amount = blockchain_service.convert_inr_to_wei(amount_inr)
    ...
    return {
        ...
        "eth_price_inr": 200000  # From settings
    }
...
async def convert_wei_to_inr(
    ...
    """Convert Wei amount to INR."""
    inr_amount = blockchain_service.convert_wei_to_inr(amount_wei)
