      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl mb-4">
            How <span className="font-brand">Mayhouse</span> works
          </h2>
          ...
          {steps.map((step, index) => (
            <div key={step.title} className="relative">
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-8 left-[calc(50%+2rem)] w-[calc(100%-4rem)] h-0.5 bg-terracotta-200" />
              )}
              <div className="relative space-y-4 text-center">
                <div className="relative inline-flex">
                  <div className="h-16 w-16 rounded-full bg-terracotta-500 flex items-center justify-center mx-auto">
                    <step.icon className="h-8 w-8 text-white" />
                  </div>
                  <div className="absolute -top-1 -right-1 h-6 w-6 rounded-full bg-white border-2 border-terracotta-500 flex items-center justify-center text-xs">
                    {index + 1}
                  </div>
                </div>
                <h3>{step.title}</h3>
